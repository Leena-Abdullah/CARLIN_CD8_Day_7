...
mcv_LA1_INT=subset(x=mcv_LA1, subset = cdr3_aa_TRA != "NA")
mcv_LA2_INT=subset(x=mcv_LA2, subset = cdr3_aa_TRA != "NA")
mcv_LA3_INT=subset(x=mcv_LA3, subset = cdr3_aa_TRA != "NA")
...
##notes some ES clones are not in the write cluster LA2_U (n=25) and LA3_U (n=27)
...
```{r}
##Let us look at AA properties, FIG 4
library(alakazam)
...
LA_ES_E_A<-LA_ES_E %>% filter(nchar(LA_ES_E$CDR3B)==15)
...
LA_ES_U_A<-LA_ES_U %>% filter(nchar(LA_ES_U$CDR3B)==15)
...
LA_ES_M_A<-LA_ES_M %>% filter(nchar(LA_ES_M$CDR3B)==15)
